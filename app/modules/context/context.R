  ...
  moduleServer(id, function(input, output, session) {
    ...
    get_tf_idf <- reactive({
      if (input$measure == "tf-idf") {
        fname <- paste0(data_dir, "context/tf-idf-data/tf_idf_", input$decade_collocates_top, "_", input$decade_collocates_bottom, "_", input$vocabulary, "_adj_noun_pairs.csv")
        fname_reverse <- paste0(data_dir, "context/tf-idf-data/tf_idf_", input$decade_collocates_bottom, "_", input$decade_collocates_top, "_", input$vocabulary, "_adj_noun_pairs.csv")
    ...
    output$collocates_top <- renderPlotly({
      ...
      if (input$measure == "count" | input$measure == "tf-idf") {
        ...
        for(d in 1:length(decades)) {
          ...
          table <- paste0(data_dir, "word_embeddings/congress_word_vectors_", fdecade, ".txt")
